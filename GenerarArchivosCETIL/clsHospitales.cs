        ...
        public String Proceso(DataTable EntidadesGenerarInformacion, Int32 CantidadArchivos)
        {
            try
            {
                ...
                if (dataRowEntidadesGenerarInformacion.Length > 0)
                {
                    ...
                    try
                    {
                        string pathCentrales = Directory.GetCurrentDirectory() + @"\IndiciosPasivocol\Hospitales";
                        ...
                        string path = Directory.GetCurrentDirectory() + @"\IndiciosPasivocol\Hospitales\Paquete_" + (cantidadDirectorios + 1).ToString();
                    ...
                    for (int i = 0; i < dataRowEntidadesGenerarInformacion.Length; i++)
                    {
                        ...
                        else
                        {
                            if (Directory.GetFiles(path).Length != 0)
                            {
                                ...
                                mensaje = string.Format("Se generaron {0} archivos de entidades hospitales", Directory.GetFiles(path).Length);
                            }
                            else
                            {
